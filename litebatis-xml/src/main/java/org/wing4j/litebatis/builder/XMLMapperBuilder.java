...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  /**
   * 全局缓存解析<cache eviction="LRU" flushInterval="60000" size="1024" readOnly="true"/> 
   * 默认情况下，如果全局开启了缓存，insert/update/delete成功后，会自动刷新相关的缓存项，
   * 但有一点要特别注意：在mybatis与hibernate混用时，由于mybatis与hibernate的缓存是无关的，
  ...
  private void cacheElement(XNode context) throws Exception {
    if (context != null) {
      ...
      //是缓存的淘汰算法，可选值有"LRU"、"FIFO"、"SOFT"、"WEAK"，缺省值是LRU
      String eviction = context.getStringAttribute("eviction", "LRU");
      Class<? extends Cache> evictionClass = typeAliasRegistry.resolveAlias(eviction);
      //指缓存过期时间，单位为毫秒，缺省值为空，即只要容量足够，永不过期
      Long flushInterval = context.getLongAttribute("flushInterval");
      //指缓存多少个对象，默认值为1024
      Integer size = context.getIntAttribute("size");
      //是否只读，如果为true，则所有相同的sql语句返回的是同一个对象（有助于提高性能，但并发操作同一条数据时，可能不安全），
